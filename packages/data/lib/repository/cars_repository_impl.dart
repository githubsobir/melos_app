...
import 'package:data/models/remote/cars/response/my_car_response.dart';
...
class CarsRepositoryImpl extends CarsRepository {
  ...
  @override
  Future<BaseResult<List<MyCarModel>>> getMyCars() async {
    try {
      var response = await _carsService.getMyCars();
      return BaseResult(
        success: true,
        body: MyCarResponse.listFromJson(response.data)
            .map(
              (e) => e.toDomainModel(),
            )
            .toList(),
  ...
  @override
  Future<BaseResult<List<CurrentCarModel>>> currentCarsOwners() async {
    try {
      var response = await _carsService.currentCarsOwners();
      return BaseResult(
        success: true,
        body: CurrentCarResponse.listFromJson(response.data)
            .map((e) => e.toDomainModel())
            .toList(),
      );
    } on DioException catch (error) {
  ...
  @override
  Future<BaseResult<bool>> changeCarStatus({required num carId}) async {
    try {
      var response = await _carsService.changeCarStatus(
        carId: carId,
      );
      return BaseResult(
        success: true,
  ...
  @override
  Future<BaseResult<CurrentGpsModel>> gpsLocation({required num id}) async {
    try {
      var response = await _carsService.gpsLocation(id: id);
      return BaseResult(
        success: true,
        body: CurrentGpsResponse.fromJson(response.data).toDomainModel(),
      );
    } on DioException catch (error) {
      return BaseResult(
          success: false, exceptionBody: error.response?.data['error_note']);
